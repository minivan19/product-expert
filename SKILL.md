...
2. **运维工单（LLM语义分析）** — 仅取去年自然年，LLM从"标题+描述+解决方案+根本原因"凝练痛点和高频模块
...
1. **运维工单 LLM 语义提取**：依赖 DeepSeek 的函数调用能力；每批15条工单，每次调用约2-3秒；诺斯贝尔128条工单约需10分钟
2. **时间筛选**：仅支持"提单时间"列的去年自然年筛选，格式支持 yyyy-mm-dd、yyyy/mm/dd
3. **DOC 读取**：30秒超时，最多读500段落+3个表格，超大文件可能截断
4. **迭代清单更新**：每年才同步一次，知识库可能滞后于实际产品迭代
